      ...
      {activeTab === 'ols' && (
        <div className="space-y-6">
          ...
          <LabBriefing
            ...
            mathExplanation="Sai số phần dư $e_i$ là khoảng cách DỌC từ điểm dữ liệu thực tế đến đường thẳng dự báo. Khi ta vẽ hình vuông có cạnh bằng $e_i$, tổng diện tích của tất cả các hình vuông này chính là $\text{RSS}$! Đường OLS tối ưu là đường thẳng duy nhất làm cho TỔNG DIỆN TÍCH CÁC HÌNH VUÔNG ĐẠT CỰC TIỂU!"
            howToInteract={[
      ...
      {activeTab === 'r2' && (
        <div className="space-y-6">
          ...
          <LabBriefing
            ...
            whatToObserve="Khi đường thẳng khớp hoàn hảo qua các điểm, $\text{RSS} = 0 \implies R^2 = 1.0\ (100\%)$. Khi đường thẳng nằm ngang ở trung bình $\bar{y}$, $R^2 = 0$!"
            takeaway="$R^2$ luôn nằm trong đoạn $[0, 1]$ đối với mô hình OLS có hệ số chặn: Càng gần $1$, mô hình càng giải thích tốt biến phụ thuộc $Y$!"
          />
        </div>
      )}
      ...
      {activeTab === 'leverage' && (
        <div className="space-y-6">
          ...
          <LabBriefing
            ...
            mathExplanation="Điểm ngoại lai có 2 loại: Outlier theo trục $Y$ (phần dư lớn) và Leverage theo trục $X$ (nằm xa trung bình $\bar{x}$). Khi một điểm VỪA nằm xa $\bar{x}$ (cánh tay đòn dài) VỪA lệch khỏi đường hồi quy, nó sẽ như một chiếc cờ lê khổng lồ bẻ gãy hoàn toàn chiều dốc của OLS!"
            howToInteract={[
      ...
      {activeTab === 'residuals' && (
        <div className="space-y-6">
          ...
          <LabBriefing
            ...
            whatToObserve="Ở mô hình Loa kèn: Càng về bên phải, các chấm phần dư càng xòe rộng ra. Điều này làm cho khoảng tin cậy và kiểm định $t\text{-test}$ không còn đáng tin cậy nữa!"
            takeaway="Kỹ năng làm bài thi và phân tích thực tế: Không bao giờ tin tưởng mù quáng vào $R^2$ cao mà phải luôn vẽ đồ thị phần dư để kiểm tra giả thiết đồng phương sai!"
          />
        </div>
      )}
    </div>
